<script>
  ...
  import { aiSettings, isAiConfigured, aiProfiles, activeProfileId } from '$lib/stores/ai-settings.js'
  import { loadSkills } from '$lib/stores/ai-skills.js'
  import { clampAiSidebarWidth, loadLayout, saveLayout } from '$lib/stores/layout.js'
  ...
  let {
    schemaContext = /** @type {any} */ ({
      schemas: [], activeSchema: 'public', tables: [], activeTable: null,
  ...
  const activeProfile = $derived($aiProfiles.find((p) => p.id === $activeProfileId) ?? null)
  const modelLabel = $derived(activeProfile?.name ?? $aiSettings.model.split('/').pop() ?? $aiSettings.model)
  ...
  async function restoreLatest() {
    ...
    items = /** @type {ChatItem[]} */ (
      (latest.items ?? []).filter(
        (i) => /** @type {any} */ (i).kind === 'user' || /** @type {any} */ (i).kind === 'assistant' ||
          /** @type {any} */ (i).kind === 'result' || /** @type {any} */ (i).kind === 'chart',
      )
    )
  ...
  async function persistCurrent() {
    const saveable = items.filter(
      (i) => i.kind === 'user' || i.kind === 'assistant' || i.kind === 'result' || i.kind === 'chart',
    )
    if (saveable.length === 0) return
    const firstUser = saveable.find((i) => i.kind === 'user')
    const title = firstUser?.kind === 'user'
  ...
  function buildViewContext() {
    ...
    if (schemaContext.activeTable) {
      lines.push(`The user is viewing table "${schemaContext.activeSchema}.${schemaContext.activeTable}". Treat it as the default subject when the request is ambiguous.`)
    }
    if (currentView === 'sql') {
      lines.push('The user has the SQL editor open. When they ask to write/fix/optimize a query, return runnable SQL in a ```sql block — the UI shows Run and "Accept ↵" buttons that insert it straight into the editor.')
      ...
      lines.push(`The user has the ORM runner open in ${ormMode} mode. When they ask for a query, return ${ormMode} JavaScript in a \`\`\`${ormMode === 'prisma' ? 'js' : 'js'} code block (use the ${ormMode} query-builder API). The UI shows an "Accept ↵" button that inserts the code into the ${ormMode} editor.`)
      ...
      lines.push('Return SQL in ```sql blocks when relevant — the user can accept it into the SQL editor.')
    }
  ...
  async function send(/** @type {string} */ [overrideText] = []) {
    ...
    const filteredCtx = filterSchemaForQuery(
      { ...schemaContext, allTableColumns: { ...schemaContext.allTableColumns, ...fetchedSchemas }, userSkills: skills },
      text,
    )
    turnSystemPrompt = buildSystemPrompt(filteredCtx) + '\n' + buildViewContext()

    const { history: managedHistory } = await manageHistory($aiSettings, apiHistory, {
      maxChars: 120_000, keepLastN: 10, summarizeThreshold: 40_000,
  ...
  async function runToolCall(call) {
    ...
    if (executedCalls.has(callKey)) {
      apiHistory.push({ role: 'tool', tool_call_id: call.id, content: JSON.stringify({ error: 'Duplicate call — use the previous result.' }) })
      return
    }
    const prior = failureTracker.get(callKey)
    if (prior && prior.count >= 2) {
      apiHistory.push({ role: 'tool', tool_call_id: call.id, content: JSON.stringify({ error: `This call failed ${prior.count} times. Do not retry.`, last_error: prior.lastError }) })
      return
    }
    ...
    try {
      ...
      if (call.function.name === 'execute_sql') {
        ...
        if (isDestructiveSql(sql)) {
          ...
          if (!ok) { apiHistory.push({ role: 'tool', tool_call_id: call.id, content: JSON.stringify({ cancelled: true, message: 'User declined.' }) }); return }
        }
        ...
        try {
          ...
          const resultId = uid()
          const resultItem = /** @type {ChatItem} */ ({ id: resultId, kind: 'result', sql, columns: cols, rows: rows.slice(0, AI_DISPLAY_ROWS), total, error: null, capped })
          const execIdx = items.findIndex((i) => i.id === execId)
          if (execIdx >= 0) items.splice(execIdx, 1, resultItem); else items.push(resultItem)
          autoOpenResult(resultId)
          await scrollBottom()
          toolResult = JSON.stringify({ columns: cols.map((c) => c.name), rows: rows.slice(0, 30), total_rows: total, message: data.message ?? null })
        } catch (sqlErr) {
          const execIdx = items.findIndex((i) => i.id === execId)
        ...
        items.push(/** @type {ChatItem} */ ({ id: schemaResultId, kind: 'result', sql: `${schema}.${table} schema`, columns: cols, rows, total: rows.length, error: null, isSchema: true }))
        ...
        const chartId = uid()
        if (!args.data?.length) {
          items.push(/** @type {ChatItem} */ ({ id: chartId, kind: 'chart', spec: args, error: 'No data provided. Call execute_sql first.' }))
          toolResult = JSON.stringify({ error: 'No data. Execute a SQL query first.' })
        } else {
          items.push(/** @type {ChatItem} */ ({ id: chartId, kind: 'chart', spec: args, error: null }))
          await scrollBottom()
  ...
  async function runSqlBlock(/** @type {string} */ sql) {
    ...
    try {
      ...
      const rows = data.rows ?? []
      const resId = uid()
      const resultItem = /** @type {ChatItem} */ ({ id: resId, kind: 'result', sql, columns: cols, rows, total: data.rowCount ?? rows.length, error: null })
      const execIdx = items.findIndex((i) => i.id === execId)
      if (execIdx >= 0) items.splice(execIdx, 1, resultItem); else items.push(resultItem)
      ...
      const errId = uid()
      const errItem = /** @type {ChatItem} */ ({ id: errId, kind: 'result', sql, columns: [], rows: [], total: 0, error: String(e) })
      const execIdx = items.findIndex((i) => i.id === execId)
      if (execIdx >= 0) items.splice(execIdx, 1, errItem); else items.push(errItem)
...
>
  ...
  <div class="absolute inset-y-0 left-0 z-20">
    <ResizeHandle
      ...
      onresize={(dx) => { width = clampAiSidebarWidth(resizeStartWidth + dx) }}
      onresizeend={() => saveLayout({ aiSidebarWidth: width })}
    />
  </div>

  <!-- Header -->
  <div class="studio-chrome flex h-9 shrink-0 items-center gap-1.5 border-b border-border px-2.5">
    <Sparkles class="size-3.5 shrink-0 text-primary" />
    <span class="min-w-0 flex-1 truncate font-mono text-ui-sm font-medium">AI Assistant</span>
  ...
  {#if historyOpen}
    ...
    <div class="absolute right-2 top-10 z-40 flex max-h-[60%] w-[min(20rem,calc(100%-1rem))] flex-col overflow-hidden rounded-lg border border-border bg-popover shadow-lg">
      ...
      <div class="app-scroll min-h-0 flex-1 overflow-y-auto p-1">
        {#if convList.length === 0}
          <p class="px-3 py-6 text-center text-ui-xs text-muted-foreground/60">No saved chats yet</p>
        {:else}
          {#each convList as conv (conv.id)}
            <div class={cn(
              ...
              <button type="button" class="flex min-w-0 flex-1 flex-col items-start gap-0.5 text-left" onclick={() => void selectConversation(conv.id)}>
                ...
                <span class="text-ui-2xs text-muted-foreground/60">{relTime(conv.updatedAt)}</span>
              </button>
              <button
                type="button"
  ...
  <div bind:this={scrollEl} class="app-scroll min-h-0 flex-1 overflow-y-auto px-3 py-3">
    {#if items.length === 0}
      <div class="flex h-full flex-col items-center justify-center gap-4 px-2 text-center">
        <div class="flex size-10 items-center justify-center rounded-xl bg-primary/10">
          <Sparkles class="size-5 text-primary" />
        </div>
        <div class="flex flex-col gap-1">
          <p class="text-ui-sm font-medium text-foreground">Ask about your database</p>
          <p class="text-ui-xs text-muted-foreground">
            {#if schemaContext.activeTable}
              Context: <span class="font-mono">{schemaContext.activeSchema}.{schemaContext.activeTable}</span>
            {:else}
      ...
      <div class="flex flex-col gap-3" data-studio-selectable="text">
        {#each items as item (item.id)}
              ...
              {#each item.parts as part, pi}
                ...
                {:else if part.type === 'sql'}
                  ...
                  <div class="overflow-hidden rounded-lg border border-border">
                    <div class="flex items-center justify-between gap-1 border-b border-border/50 bg-muted/40 px-2 py-1">
                      <button type="button" class="flex items-center gap-1 text-ui-2xs text-muted-foreground hover:text-foreground" onclick={() => toggleCollapse(sqlKey)}>
                        {#if sqlOpen}<ChevronDown class="size-3" />{:else}<ChevronRight class="size-3" />{/if}
                        <span class="font-mono">SQL</span>
                      </button>
                      <div class="flex items-center gap-0.5">
                        <button type="button" class="inline-flex h-6 items-center gap-1 rounded px-1.5 text-ui-2xs text-muted-foreground hover:bg-accent hover:text-foreground" onclick={() => copyText(part.content)} title="Copy">
                          <Copy class="size-3" />
                        </button>
                        <button type="button" class="inline-flex h-6 items-center gap-1 rounded px-1.5 text-ui-2xs text-muted-foreground hover:bg-accent hover:text-foreground" onclick={() => acceptSql(part.content)} title="Accept into editor">
                          <CornerDownLeft class="size-3" />Accept
                        </button>
                        <button type="button" class="inline-flex h-6 items-center gap-1 rounded bg-primary px-1.5 text-ui-2xs text-primary-foreground hover:opacity-90 disabled:opacity-50" disabled={loading} onclick={() => void runSqlBlock(part.content)} title="Run">
                ...
                {:else if part.type === 'mermaid'}
                  <div class="overflow-hidden rounded-lg border border-border">
                    <div class="flex items-center justify-between gap-1 border-b border-border/50 bg-muted/40 px-2 py-1">
                      ...
                      <button type="button" class="inline-flex h-6 items-center gap-1 rounded px-1.5 text-ui-2xs text-muted-foreground hover:bg-accent hover:text-foreground" onclick={() => copyText(part.content)}>
                ...
                {:else if part.type === 'error'}
                  <div class="flex items-start gap-2 rounded-lg border border-destructive/30 bg-destructive/8 px-3 py-2 text-ui-xs text-destructive">
                    <AlertTriangle class="mt-0.5 size-3.5 shrink-0" />
                    <span>{part.content}</span>
                  </div>
                {:else if part.type === 'confirm_prompt'}
                  <div class="flex items-start gap-2 rounded-lg border border-amber-500/30 bg-amber-500/8 px-3 py-2 text-ui-xs text-amber-600 dark:text-amber-400">
                    <AlertTriangle class="mt-0.5 size-3.5 shrink-0" />
                    <span>{part.content}</span>
                  </div>
                {:else}
                  ...
                  <div class="overflow-hidden rounded-lg border border-border">
                    <div class="flex items-center justify-between gap-1 border-b border-border/50 bg-muted/40 px-2 py-1">
                      ...
                      <div class="flex items-center gap-0.5">
                        ...
                        <button type="button" class="inline-flex h-6 items-center gap-1 rounded px-1.5 text-ui-2xs text-muted-foreground hover:bg-accent hover:text-foreground" onclick={() => acceptCode(part.lang, part.content)} title="Accept into editor">
                          <CornerDownLeft class="size-3" />Accept
                        </button>
                      </div>
                    </div>
          ...
          {:else if item.kind === 'executing'}
            <div class="flex items-center gap-2 rounded-lg border border-border/60 bg-muted/20 px-2.5 py-1.5 text-ui-2xs text-muted-foreground">
          ...
          {:else if item.kind === 'result'}
            ...
            <div class={cn('overflow-hidden rounded-lg border text-ui-2xs', item.error ? 'border-destructive/40 bg-destructive/5' : item.isSchema ? 'border-primary/25' : 'border-border/70')}>
              <button type="button" class={cn('flex w-full items-center gap-1.5 px-2.5 py-1.5 text-left transition-colors hover:bg-muted/20', item.error ? 'bg-destructive/8' : item.isSchema ? 'bg-primary/6' : 'bg-muted/20', resOpen && 'border-b border-border/40')} onclick={() => toggleResult(item.id)}>
                ...
                {#if !item.error}
                  <span class="shrink-0 rounded-full bg-muted/60 px-1.5 py-0.5 font-mono text-[9px] tabular-nums text-muted-foreground">{formatCompactCount(item.total)} {item.total === 1 ? 'row' : 'rows'}</span>
                {/if}
              </button>
              {#if resOpen}
                {#if item.error}
                ...
                {:else if item.rows.length === 0}
                  <p class="px-2.5 py-2.5 text-center text-ui-2xs italic text-muted-foreground/60">No rows returned.</p>
                {:else}
                  <div class="overflow-x-auto">
                    <DataTable columns={item.columns} rows={item.rows.slice(0, 15)} embedded showSelection={false} />
                  </div>
                  {#if item.total > 15}
                    <p class="border-t border-border/20 px-2.5 py-1 text-[10px] text-muted-foreground/50">Showing 15 of {formatCompactCount(item.total)} rows{item.capped ? ` (capped at ${AI_ROW_LIMIT})` : ''}</p>
                  {/if}
                {/if}
              {/if}
            </div>
          ...
          {:else if item.kind === 'confirm'}
            <div class="overflow-hidden rounded-lg border border-destructive/40 bg-destructive/5">
              <div class="flex items-center gap-2 border-b border-destructive/30 bg-destructive/10 px-2.5 py-1.5">
              ...
              <div class="flex items-center justify-end gap-2 border-t border-destructive/20 px-2.5 py-1.5">
          ...
          {:else if item.kind === 'chart'}
            <div class="overflow-hidden rounded-lg border border-border bg-card">
              {#if item.error}
                <div class="flex items-center gap-2 px-3 py-2 text-ui-xs text-destructive">
    ...
    {#if error}
      <div class="mt-3 flex items-start gap-2 rounded-lg border border-destructive/30 bg-destructive/8 px-3 py-2 text-ui-xs text-destructive">
        <AlertTriangle class="mt-0.5 size-3.5 shrink-0" />
  ...
  <div class="shrink-0 border-t border-border bg-panel p-2.5">
    <div class="flex items-end gap-1.5 rounded-xl border border-border bg-background px-2.5 py-1.5 focus-within:border-ring/60">
      <textarea
        bind:this={inputRef}
        ...
        rows="1"
        placeholder={configured ? 'Ask anything about your data…' : 'Configure a model first'}
        disabled={!configured}
      ...
      ></textarea>
      {#if loading}
        <button type="button" class="inline-flex size-7 shrink-0 items-center justify-center rounded-lg bg-muted text-foreground transition-colors hover:bg-muted/70" onclick={stop} title="Stop">
          <Square class="size-3.5" />
        </button>
      {:else}
        <button type="button" class="inline-flex size-7 shrink-0 items-center justify-center rounded-lg bg-primary text-primary-foreground transition-opacity hover:opacity-90 disabled:opacity-40" disabled={!inputText.trim() || !configured} onclick={() => void send()} title="Send">
          <Send class="size-3.5" />
        </button>
      {/if}
    </div>
