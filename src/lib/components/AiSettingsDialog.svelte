<script>
  ...
  import {
    aiProfiles, activeProfileId, aiSettings,
    saveProfile, deleteProfile, setActiveProfile,
  ...
  async function startEdit(profile) {
    ...
    testState = 'idle'
    testMsg = ''
    try { formApiKey = await invoke('ai_load_key', { profileId: profile.id }) } catch { formApiKey = '' }
    view = 'edit'
  }
  ...
  async function testConnection() {
    ...
    try {
      const settings = { baseUrl: formBaseUrl || (provider?.url ?? ''), apiKey: formApiKey, model: formModel }
      const res = await chatCompletionRaw(settings, [{ role: 'user', content: 'Reply with the single word: ok' }], null)
...
<Dialog.Root bind:open onOpenChange={handleOpenChange}>
  <Dialog.Content class="gap-0 overflow-hidden p-0 sm:max-w-[26rem]">
    <Dialog.Header class="flex shrink-0 items-center justify-between border-b border-border/60 px-4 py-3">
      <Dialog.Title class="text-sm font-semibold">
        {#if view === 'list'}AI Models{:else if view === 'add'}Add model{:else}Edit model{/if}
      </Dialog.Title>
      {#if view !== 'list'}
        <button
          type="button"
          class="flex size-6 items-center justify-center rounded text-muted-foreground hover:text-foreground"
          onclick={() => { view = 'list'; editingId = null }}
    ...
    <!-- ── LIST VIEW ──────────────────────────────────────────────────────── -->
    {#if view === 'list'}
      <div class="app-scroll flex max-h-[min(70vh,36rem)] flex-col gap-1 overflow-y-auto p-3">
        {#each $aiProfiles as profile (profile.id)}
          {@const isActive = profile.id === $activeProfileId}
          <div
            class={cn(
              'group flex items-center gap-2.5 rounded-lg border px-3 py-2.5 transition-colors cursor-pointer',
              isActive ? 'border-primary/30 bg-primary/5' : 'border-border hover:border-border/80 hover:bg-accent/40',
            )}
          ...
          >
            <!-- Active indicator -->
            <div class={cn('size-2 shrink-0 rounded-full', isActive ? 'bg-primary' : 'bg-border group-hover:bg-muted-foreground/40')}></div>
            <!-- Info -->
            <div class="min-w-0 flex-1">
              <p class="truncate text-ui-xs font-medium text-foreground">{profile.name}</p>
              <p class="truncate font-mono text-[10px] text-muted-foreground/70">{profile.model}</p>
            </div>
            <!-- Provider badge -->
            <span class="shrink-0 rounded px-1.5 py-0.5 text-[9px] font-medium uppercase tracking-wide text-muted-foreground/50 bg-muted/50">
              {profile.provider}
            </span>
            <!-- Edit button -->
            <button
              type="button"
              class="shrink-0 rounded p-1 text-muted-foreground/40 opacity-0 transition-opacity hover:text-foreground group-hover:opacity-100"
        ...
        <button
          type="button"
          class="mt-1 flex items-center gap-2 rounded-lg border border-dashed border-border/60 px-3 py-2.5 text-ui-xs text-muted-foreground transition-colors hover:border-border hover:text-foreground"
          onclick={startAdd}
        >
          <Plus class="size-3.5" />
          Add model
    ...
    <!-- ── ADD / EDIT VIEW ───────────────────────────────────────────────── -->
    {:else}
      <div class="app-scroll flex max-h-[min(70vh,40rem)] flex-col gap-4 overflow-y-auto px-4 py-4">

        <!-- Provider picker -->
        <div class="flex flex-col gap-2">
          <p class="text-[10px] font-medium uppercase tracking-wider text-muted-foreground/60">Provider</p>
        ...
        {#if modelPresets.length > 0}
          <div class="flex flex-col gap-2">
            <p class="text-[10px] font-medium uppercase tracking-wider text-muted-foreground/60">Model</p>
            <div class="grid grid-cols-2 gap-1.5">
              {#each modelPresets as preset (preset.model)}
                ...
                >
                  <span class="min-w-0 truncate text-[11px] font-medium text-foreground">{preset.label}</span>
                  <span class={cn('shrink-0 rounded px-1 py-0.5 font-mono text-[9px]', tagClass[preset.tag])}>{preset.tag}</span>
                </button>
              {/each}
            </div>
          </div>
        ...
        <div class="flex flex-col gap-3">
          ...
          <div class="flex flex-col gap-1.5">
            <label for="form-model" class="text-ui-xs font-medium text-foreground">{modelPresets.length > 0 ? 'Model (editable)' : 'Model ID'}</label>
          ...
          <div class="flex flex-col gap-1.5">
            ...
            <Input id="form-name" class="h-8 text-ui-xs" placeholder={formModel ? modelShortName(formModel) : 'My model'} bind:value={formName} />
          </div>

          {#if !isOllama}
            <div class="flex flex-col gap-1.5">
              <label for="form-key" class="text-ui-xs font-medium text-foreground">
                API key
                <span class="ml-1 font-normal text-muted-foreground/60">(stored securely, not in browser)</span>
              </label>
              ...
              {#if needsKey}
                <p class="flex items-center gap-1 text-[10px] text-amber-600 dark:text-amber-400">
                  <AlertTriangle class="size-3 shrink-0" />
                  API key required for this provider.
                  {#if provider?.keysUrl}
                    <button type="button" class="font-medium underline-offset-2 hover:underline" onclick={() => openExternal(provider.keysUrl)}>
                      Get one →
                    </button>
                  {/if}
                </p>
          ...
          {#if !isCustom && !isOllama}
            ...
            >
              <ChevronRight class={cn('size-3 transition-transform', showAdvanced && 'rotate-90')} />
              Custom endpoint
            </button>
            {#if showAdvanced}
              <div class="flex flex-col gap-1.5">
        ...
        <!-- Test result -->
        {#if testState === 'ok'}
          <div class="flex items-center gap-2 rounded-lg border border-green-500/30 bg-green-500/8 px-3 py-2 text-ui-xs text-green-600 dark:text-green-400">
            <Check class="size-3.5 shrink-0" /><span class="min-w-0 truncate">{testMsg}</span>
          </div>
        {:else if testState === 'error'}
          <div class="flex items-start gap-2 rounded-lg border border-destructive/30 bg-destructive/8 px-3 py-2 text-ui-xs text-destructive">
            <AlertTriangle class="mt-0.5 size-3.5 shrink-0" /><span class="min-w-0 break-words">{testMsg}</span>
          </div>
        {/if}
      </div>
      ...
      <div class="flex items-center justify-between gap-2 border-t border-border/60 px-4 py-3">
        <div class="flex items-center gap-2">
          <Button type="button" variant="outline" size="sm" class="h-8 gap-1.5 text-ui-xs"
          ...
          {#if view === 'edit' && editingId}
            <Button type="button" variant="ghost" size="sm" class="h-8 gap-1.5 text-ui-xs text-destructive hover:text-destructive"
              onclick={() => void handleDelete(editingId)}
            ><Trash2 class="size-3.5" /></Button>
          {/if}
        </div>
        <Button type="button" size="sm" class="h-8 gap-1.5 text-ui-xs"
