<script>
  ...
  const TYPE_LABEL = /** @type {Record<string,string>} */ ({
    connect: 'Connect', disconnect: 'Disconnect', table_open: 'Table',
    row_fetch: 'Fetch', row_save: 'Save', row_delete: 'Delete',
    row_insert: 'Insert', sql_exec: 'SQL', export: 'Export',
    schema_change: 'Schema', filter: 'Filter', error: 'Error',
  })

  const filtered = $derived.by(() => {
    let list = entries
    if (typeFilter === 'error') list = list.filter((e) => !e.success)
    else if (typeFilter === 'connect') list = list.filter((e) => e.type === 'connect' || e.type === 'disconnect')
    else if (typeFilter !== 'all') list = list.filter((e) => e.type === typeFilter)
  ...
  const avgMs = $derived(
    timedEntries.length > 0
      ? Math.round(timedEntries.reduce((s, e) => s + (e.durationMs ?? 0), 0) / timedEntries.length)
      : null
  )
...
<div class="flex min-h-0 flex-1 flex-col overflow-hidden">
  ...
  <div class="studio-chrome flex h-9 shrink-0 items-center gap-2 border-b border-border bg-panel px-3">
    ...
    {#if entries.length > 0}
      <span class="rounded-full bg-muted px-2 py-px font-mono text-ui-2xs text-muted-foreground">{entries.length}</span>
    {/if}
  ...
  <!-- Filter toolbar -->
  <div class="flex shrink-0 flex-wrap items-center gap-2 border-b border-border/60 bg-panel px-3 py-2">
    <!-- Search -->
    ...
    {#if filtered.length === 0}
      <div class="flex h-full flex-col items-center justify-center gap-3 text-center">
        ...
        <p class="text-ui-xs text-muted-foreground/60">
          {entries.length === 0 ? 'Operations will appear here as you use the app' : 'Try a different filter'}
        </p>
      </div>
    {:else}
      {#each filtered as entry (entry.id)}
        ...
        <div
          class={cn(
            ...
            entry.success ? 'hover:bg-accent/25' : 'bg-destructive/5 hover:bg-destructive/10',
        ...
        >
          ...
          >
            ...
            {:else}
              <CheckCircle2 class="size-3.5 shrink-0 text-muted-foreground/20" />
            {/if}

            <!-- Type chip -->
            <span class={cn('inline-flex shrink-0 items-center gap-1 rounded px-1.5 py-px font-mono text-ui-3xs font-medium', chipClass(entry))}>
              <Icon class="size-2.5" />
            ...
            <!-- Table badge -->
            {#if entry.table}
              <span class="shrink-0 rounded bg-muted px-1.5 py-px font-mono text-ui-2xs text-muted-foreground">
            ...
            {#if entry.rowCount != null}
              <span class="shrink-0 font-mono text-ui-2xs tabular-nums text-muted-foreground/60">{entry.rowCount.toLocaleString()}r</span>
            {/if}

            <!-- Duration -->
            {#if entry.durationMs != null}
              <span class="w-12 shrink-0 text-right font-mono text-ui-2xs tabular-nums text-muted-foreground/50">{entry.durationMs}ms</span>
            {/if}

            <!-- Time -->
            <span class="w-[58px] shrink-0 text-right font-mono text-ui-2xs tabular-nums text-muted-foreground/40">{fmtTime(entry.timestamp)}</span>

            <!-- Expand chevron -->
            {#if hasDetail}
              <ChevronRight class={cn('size-3 shrink-0 text-muted-foreground/40 transition-transform', isExpanded && 'rotate-90')} />
            {:else}
          ...
          {#if isExpanded}
            <div class="mx-2 mb-2 rounded-md border border-border/50 bg-card">
              <div class="flex items-center gap-3 border-b border-border/40 px-3 py-1.5 font-mono text-ui-2xs text-muted-foreground/60">
                <span>{new Date(entry.timestamp).toLocaleString()}</span>
                {#if entry.durationMs != null}<span>{entry.durationMs}ms</span>{/if}
                {#if entry.schema || entry.table}<span>{[entry.schema, entry.table].filter(Boolean).join('.')}</span>{/if}
              </div>
              {#if entry.detail}
                <pre class="app-scroll max-h-40 overflow-auto px-3 py-2.5 font-mono text-ui-xs leading-relaxed text-foreground/90 [white-space:pre-wrap]">{entry.detail}</pre>
              {/if}
              {#if entry.error}
                <pre class="app-scroll max-h-28 overflow-auto px-3 py-2.5 font-mono text-ui-xs leading-relaxed text-destructive [white-space:pre-wrap]">{entry.error}</pre>
              {/if}
            </div>
          {/if}
        </div>
  ...
  <!-- Status bar -->
  {#if entries.length > 0}
    <div class="flex shrink-0 items-center gap-3 border-t border-border/40 bg-panel px-3 py-1">
      <span class="font-mono text-ui-2xs text-muted-foreground/50 tabular-nums">{filtered.length} of {entries.length} events</span>
      {#if avgMs !== null}
        <span class="font-mono text-ui-2xs text-muted-foreground/40">avg {avgMs}ms</span>
      {/if}
    </div>
  {/if}
</div>
