<script>
  ...
  function toRecords(result) {
    ...
    return result.rows.map((row) => /** @type {Record<string, unknown>} */ (Object.fromEntries(cols.map((c, i) => [c, row[i]]))))
  }
  ...
  async function loadRoles() {
    ...
    try {
      ...
      `))
    } catch (e) { rolesError = String(e) } finally { rolesLoading = false }
  }
  ...
  async function loadPolicies() {
    ...
    try {
      ...
      `))
    } catch (e) { policiesError = String(e) } finally { policiesLoading = false }
  }
  ...
  async function loadRls() {
    ...
    try {
      ...
      `))
    } catch (e) { rlsError = String(e) } finally { rlsLoading = false }
  }
  ...
  async function runSqlModal() {
    ...
    try {
      ...
      sqlModal = null
      refresh()
    } catch (e) { toast.error('Execution failed', { description: String(e) }) }
    finally { sqlRunning = false }
  }

  function openRoleModal(/** @type {Record<string,unknown> | null} */ role = null) {
    if (role) {
...
<div class="flex min-h-0 flex-1 flex-col overflow-hidden">
  ...
  <!-- Header -->
  <div class="studio-chrome flex h-9 shrink-0 items-center gap-2 border-b border-border bg-panel px-3">
  ...
  <div class="flex shrink-0 items-center border-b border-border bg-panel px-3 py-2">
    <div class="inline-flex h-7 items-center rounded-md border border-border/60 bg-muted/40 p-0.5 ring-1 ring-inset ring-border/40">
  ...
  <div class="min-h-0 flex-1 overflow-auto">
    ...
    {#if activeTab === 'roles'}
      <div class="flex items-center justify-between border-b border-border/40 px-3 py-2">
        <span class="font-mono text-ui-xs text-muted-foreground">{roles?.length ?? 0} roles</span>
        <button
          type="button"
        ...
        <table class="w-full text-ui-xs">
          <thead class="studio-chrome sticky top-0 z-10 bg-panel text-left">
            <tr class="border-b border-border/50">
              <th class="px-3 py-2 font-mono font-normal text-muted-foreground">Name</th>
              <th class="px-3 py-2 text-center font-mono font-normal text-muted-foreground" title="Can login">Login</th>
              <th class="px-3 py-2 text-center font-mono font-normal text-muted-foreground" title="Superuser">Super</th>
              <th class="px-3 py-2 text-center font-mono font-normal text-muted-foreground" title="Create DB">CreateDB</th>
              <th class="px-3 py-2 text-center font-mono font-normal text-muted-foreground" title="Create Role">CreateRole</th>
              <th class="px-3 py-2 text-center font-mono font-normal text-muted-foreground" title="Bypass RLS">BypassRLS</th>
              <th class="px-3 py-2 font-mono font-normal text-muted-foreground">Conn limit</th>
              <th class="px-3 py-2 font-mono font-normal text-muted-foreground">Expires</th>
          ...
          <tbody>
            {#each (roles ?? []) as role (role.rolname)}
              ...
              <tr class="group/row border-b border-border/30 outline-none hover:bg-accent/25">
                <td class="px-3 py-2">
                  <div class="flex items-center gap-2">
                    <span class={cn(
                      ...
                      isLogin ? 'bg-primary/10 text-primary' :
                      'bg-muted text-muted-foreground',
                    )}>{String(role.rolname).slice(0, 2)}</span>
                    <span class="font-mono text-ui-xs font-medium text-foreground">{role.rolname}</span>
                    {#if isSuper}
                      <span class="rounded bg-amber-500/10 px-1.5 py-px font-mono text-ui-3xs font-medium text-amber-600 dark:text-amber-400">superuser</span>
                    {/if}
                  </div>
                </td>
                {#each [
                  { col: 'rolcanlogin', c: 'text-primary' },
                  { col: 'rolsuper',    c: 'text-amber-500' },
                  { col: 'rolcreatedb', c: 'text-primary' },
                  { col: 'rolcreaterole', c: 'text-primary' },
                  { col: 'rolbypassrls',  c: 'text-primary' },
                ] as f (f.col)}
                ...
                <td class="px-3 py-2 font-mono text-ui-xs text-muted-foreground/60">{role.rolvaliduntil ?? '—'}</td>
                <td class="px-2 py-2">
                  <button
                    type="button"
                    ...
                    title="Edit"
                  ><KeyRound class="size-3.5" /></button>
                </td>
              </tr>
            {/each}
          </tbody>
    ...
    {:else if activeTab === 'policies'}
      <div class="flex items-center justify-between border-b border-border/40 px-3 py-2">
        <span class="font-mono text-ui-xs text-muted-foreground">{policies?.length ?? 0} polic{(policies?.length ?? 0) === 1 ? 'y' : 'ies'}</span>
        <button
          type="button"
          class="inline-flex h-7 items-center gap-1.5 rounded-md border border-border bg-background px-2.5 font-mono text-ui-xs text-muted-foreground transition-colors hover:bg-accent/35 hover:text-foreground"
      ...
      {:else if policiesError}
        <div class="px-4 py-8 text-center">
          ...
          <button onclick={() => { policies = null; loadPolicies() }} class="mt-2 font-mono text-ui-xs text-muted-foreground underline">Retry</button>
        </div>
      {:else if policies.length === 0}
        <div class="flex h-full flex-col items-center justify-center gap-3 py-20 text-center">
          ...
          <p class="font-mono text-ui text-muted-foreground">No policies found</p>
          <p class="mt-1 text-ui-xs text-muted-foreground/60">Enable RLS on a table and create a policy to control row access</p>
          <button
            type="button"
      ...
      {:else}
        <table class="w-full text-ui-xs">
          <thead class="studio-chrome sticky top-0 z-10 bg-panel text-left">
            <tr class="border-b border-border/50">
              <th class="px-3 py-2 font-mono font-normal text-muted-foreground">Table</th>
              <th class="px-3 py-2 font-mono font-normal text-muted-foreground">Policy</th>
              <th class="px-3 py-2 font-mono font-normal text-muted-foreground">Command</th>
              <th class="px-3 py-2 font-mono font-normal text-muted-foreground">Roles</th>
              <th class="w-28 px-3 py-2 font-mono font-normal text-muted-foreground">Type</th>
              <th class="w-9 px-2"></th>
            </tr>
          </thead>
          <tbody>
            {#each (policies ?? []) as pol, i (i)}
              {@const permissive = (pol.permissive ?? '').toString().toUpperCase() === 'PERMISSIVE'}
              <tr class={cn('group/row border-b border-border/30 outline-none hover:bg-accent/25', expandedPolicy === i && 'bg-accent/15')}>
                ...
                <td class="px-3 py-2">
                  <span class="rounded bg-muted px-1.5 py-px font-mono text-ui-2xs text-muted-foreground">{pol.cmd}</span>
                </td>
                <td class="max-w-[100px] truncate px-3 py-2 font-mono text-muted-foreground/70">{pol.roles || 'public'}</td>
                <td class="px-3 py-2">
                  <span class={cn('rounded px-1.5 py-px font-mono text-ui-3xs font-medium',
                    permissive ? 'bg-primary/10 text-primary' : 'bg-orange-500/10 text-orange-600 dark:text-orange-400'
                ...
                <td class="px-2 py-2">
                  ...
                  ><ChevronDown class={cn('size-3.5 transition-transform', expandedPolicy === i && 'rotate-180')} /></button>
                </td>
              </tr>
              {#if expandedPolicy === i}
                <tr class="border-b border-border/30 bg-muted/20">
                  <td colspan="6" class="px-4 py-3">
                    <div class={cn('grid gap-3', pol.qual && pol.with_check ? 'grid-cols-2' : 'grid-cols-1')}>
                      {#if pol.qual}
                        <div>
                          <p class="mb-1.5 font-mono text-ui-2xs text-muted-foreground/60 uppercase tracking-wider">USING</p>
                          <pre class="rounded-md border border-border/40 bg-background px-3 py-2 font-mono text-ui-xs leading-relaxed text-foreground/90">{pol.qual}</pre>
                        </div>
                      {/if}
                      {#if pol.with_check}
                        <div>
                          <p class="mb-1.5 font-mono text-ui-2xs text-muted-foreground/60 uppercase tracking-wider">WITH CHECK</p>
                          <pre class="rounded-md border border-border/40 bg-background px-3 py-2 font-mono text-ui-xs leading-relaxed text-foreground/90">{pol.with_check}</pre>
                        </div>
                      {/if}
                    </div>
                  </td>
    ...
    {:else if activeTab === 'rls'}
      ...
      <div class="flex items-center gap-3 border-b border-border/40 px-3 py-2">
        ...
        {#if rlsTables && rlsTables.length > 0}
          <div class="h-1 flex-1 overflow-hidden rounded-full bg-muted">
            <div class="h-full rounded-full bg-primary/50 transition-all" style="width:{Math.round(enabledCount/rlsTables.length*100)}%"></div>
          </div>
        {/if}
      </div>
      ...
      {:else if rlsError}
        <div class="px-4 py-8 text-center">
          ...
          <button onclick={() => { rlsTables = null; loadRls() }} class="mt-2 font-mono text-ui-xs text-muted-foreground underline">Retry</button>
        </div>
      {:else}
        <table class="w-full text-ui-xs">
          <thead class="studio-chrome sticky top-0 z-10 bg-panel text-left">
            <tr class="border-b border-border/50">
              <th class="px-3 py-2 font-mono font-normal text-muted-foreground">Schema</th>
              <th class="px-3 py-2 font-mono font-normal text-muted-foreground">Table</th>
              <th class="px-3 py-2 text-center font-mono font-normal text-muted-foreground">RLS</th>
              <th class="px-3 py-2 text-center font-mono font-normal text-muted-foreground">Force RLS</th>
          ...
          <tbody>
            {#each (rlsTables ?? []) as row (`${row.schema}.${row.table_name}`)}
              ...
              <tr class="group/row border-b border-border/30 outline-none hover:bg-accent/25">
                <td class="px-3 py-2 font-mono text-muted-foreground/60">{row.schema}</td>
                <td class="px-3 py-2 font-mono font-medium text-foreground">{row.table_name}</td>
                <td class="px-3 py-2 text-center">
                  {#if enabled}
                    <span class="inline-flex items-center gap-1 rounded-full bg-primary/10 px-2 py-px font-mono text-ui-3xs font-medium text-primary">
                      <Lock class="size-2.5" />On
                    </span>
                  {:else}
                    <span class="inline-flex items-center gap-1 rounded-full bg-muted px-2 py-px font-mono text-ui-3xs text-muted-foreground/60">
